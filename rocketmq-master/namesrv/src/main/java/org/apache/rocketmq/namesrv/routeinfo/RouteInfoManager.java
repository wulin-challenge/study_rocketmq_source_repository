...
/**
 * 路由元信息
 * <p> RouteInfoManager实例中保存了整个消息集群的相关信息，这个是关键类
 * 
 * <p> 详解: RocketMQ基于订阅发布机制,一个Topic拥有多个消息队列,一个Broker为每一主题默认创建4个读队列4个写队列。
 * 多个Broker组成一个集群,BrokerName由相同的多台Broker组成Master-Slave架构,brokerId为0代表Master,大于0表示Slave。
 * BrokerLivelnfo中的lastUpdateTimestamp存储上次收到Broker心跳包的时间。
 *
 */
public class RouteInfoManager {
     ...
     * 
     * <p> 疑惑 : topicQueueTable有一点不明白，为什么一个topic对应的是一个List，难道不是一个topic对应一个QueueData吗？难道可以存在相同的topic？
     * <p> 解答: 因为每一个topic可以存在不同的broker中，不同的broker就有不同的QueueData。
     */
    private final HashMap<String/* topic */, List<QueueData>> topicQueueTable;
    
    /**
     * Broker 基础信息,包含 brokerName,所属集群名称,主备Broker地址 。
     * <p> 以Broker Name为单位的Broker集合 
     * <p> 以Broker Name为Key，BrokerData为Value：
     * 
     * <p> 疑问: brokerAddrTable根据一个brokername获得一个BrokerData，可是为什么一个BrokerData中有一个hashMap的brokerAddrs呢？
     * 我能理解的是一个brokername，会有多个address和id，但是cluster为什么又是同一个呢？
     * 
     * <p> 解答: 这是因为你理解错了，以为BrokerName是唯一的，实际上是指定多个broker为一个name，只不过指定了不同的id和addres来区分是master还是slave。
     */
    private final HashMap<String/* brokerName */, BrokerData> brokerAddrTable;
    
    /**
     * Broker集群信息,存储集群中所有Broker 名称
     * <p> 根据一个集群名对应的一组BrokerName的列表
     */
    private final HashMap<String/* clusterName */, Set<String/* brokerName */>> clusterAddrTable;
    
    /**
     * 存活的Broker地址列表 
     * <p> Broker状态信息,NameServer每次收到心跳包时会替换该信息
     * 
     * <p> 用于存放存活的Broker信息
     */
     ...
     * 
     * <p> Broker上的FilterServer列表,用于类模式消息过滤
     */
    ...
    /**
     * 检测判断当前 brokerAddr 在  brokerLiveTable(存活的Broker地址列表) 中是否发生变化,若发生变化则返回true,否则返回false
     * @param brokerAddr 要检测判断的 broker地址
     ...
     * 创建或者更新Topic消息队列路由信息
     * @param brokerName broker名称
     * @param topicConfig 从broker心跳发送过来的主题配置信息
     */
    ...
    /**
     * 从
     * <p> brokerLiveTable  (存活的Broker地址列表 ),
     * <p> filterServerTable(Broker上的FilterServer列表,用于类模式消息过滤),
     * <p> brokerAddrTable  (Broker 基础信息,包含 brokerName,所属集群名称,主备Broker地址 。),
     * <p> clusterAddrTable (根据一个集群名对应的一组BrokerName的列表),
     * <p> topicQueueTable  (Topic 消息队列路由信息，消息发送时根据路由表进行负载均衡 。 )
     * <p> 中移除与 remoteAddr 和  channel 相关的被销毁的数据
     * @param remoteAddr 要销毁的远程地址
     * @param channel 要销毁的远程netty管道
     */
