 ...
 * <p/>
 * 
 * <p> 消费队列的扩展，存储不重要的东西，如消息存储时间，过滤位图等。
 * 
 * <li>1. This class is used only by {@link ConsumeQueue}</li>
 * 
 * <li> 此类仅由ConsumeQueue使用
 * 
...
public class ConsumeQueueExt {
     ...
     * @param queueId id of queue - 队列的ID
     * @param storePath root dir of files to store. - 要存储的文件的根目录。
     * @param mappedFileSize file size - 文件大小
     * @param bitMapLength bit map length. - 位map长度。
     */
     ...
     * <p> 检查地址是否指向扩展文件。
     * 
     * <p> Just test {@code address} is less than 0.
     * 
     * <p> 只是测试地址小于0。
     */
     ...
     * 
     * <p> 将地址（由decorate修饰）转换为映射文件中的偏移量。
     * 
     * <p> if {@code address} is less than 0, return {@code address} - {@link java.lang.Long#MIN_VALUE};
     * else, just return {@code address}
     * 
     * <p> 如果address小于0，则返回地址 -  java.lang.Long.MIN_VALUE; 否则，只是返回地址
     */
     ...
     * 
     * <p> 从映射文件中提取偏移量，以便与tagsCode（最初保存在cq中）区分开来。
     * 
     * <p> if {@code offset} is greater than or equal to 0, then return {@code offset} + {@link java.lang.Long#MIN_VALUE};
     * else, just return {@code offset}
     * 
     * <p> 如果offset大于或等于0，则返回offset + java.lang.Long.MIN_VALUE; 否则，只返回偏移量
     *
     ...
     * <p> 保存到文件的映射缓冲区并返回地址。
     * 
     * <p> Be careful, this method is not thread safe.
     * 
     * <p> 注意，这种方法不是线程安全的。
     * 
     *
